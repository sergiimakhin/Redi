...
selected_dummies = correlations[correlations > 0.45].index

# Keep all filtered dummies as separate features
final_dummies = df_dummies[selected_dummies]

# Combine with numeric features
final_df = pd.concat([df3.select_dtypes(include='number'), final_dummies], axis=1)

# Fit model (e.g., LinearRegression or Lasso)
from sklearn.linear_model import LinearRegression
model = LinearRegression()
model.fit(final_df.drop('SalePrice', axis=1), final_df['SalePrice'])
```
